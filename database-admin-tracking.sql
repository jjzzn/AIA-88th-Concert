...
CREATE TABLE IF NOT EXISTS admin_action_logs (
    ...
    action_type VARCHAR(50) NOT NULL, -- 'seat_swap', 'check_in', 'search', 'view_dashboard'
    booking_id UUID REFERENCES bookings(id) ON DELETE SET NULL,
    booking_seat_id UUID REFERENCES booking_seats(id) ON DELETE SET NULL,
    details JSONB, -- Store additional details like old_seat, new_seat, search_term, etc.
    ip_address VARCHAR(50),
    user_agent TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Admin Sessions Table (for login tracking)
...
-- Insert sample admin users (CHANGE PASSWORDS IN PRODUCTION!)
-- Password for all: 'admin123' (hashed with bcrypt)
INSERT INTO admin_users (username, full_name, email, password_hash, role) VALUES
('admin1', 'Admin User 1', 'admin1@aia.com', '$2a$10$YourHashedPasswordHere1', 'admin'),
('admin2', 'Admin User 2', 'admin2@aia.com', '$2a$10$YourHashedPasswordHere2', 'admin'),
('superadmin', 'Super Admin', 'superadmin@aia.com', '$2a$10$YourHashedPasswordHere3', 'superadmin')
...
-- View for admin action history with user details
